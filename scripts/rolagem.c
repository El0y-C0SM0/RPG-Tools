...
void info() {
    ...
    puts("Este é um programa feito em linguagem C com o intuito de ajudar jogadores de rpg's de todos os tipo. O programa realiza rolagem de dados dos tipos d2, d3, d4, d6, d8, d10, d12, d20, d100 e d1000; podendo ser solicitado a rolagem de mais de um tipo no mesmo comando e com uma quantidade segura de até 100 dados de cada tipo. Para mais informações de como dar os comandos digite: -help\n");
}

int main() {
    char prompt[15];

    puts("Bem vindo ao sistema de rolagem de dados!");
    ...
    while (scanf(" %s", prompt) != EOF) {
        if(strcmp(prompt, "-roll") == 0) {
            ...
            printa_resultados(lance);
        } else if(strcmp(prompt, "-roll-arma") == 0) {
            rola_dano_arma();
