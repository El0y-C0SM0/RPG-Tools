...
int calc() {
    ...
    case '%':
        ...
        break;
    default:
        puts("operação não reconhecida! Digite \"-help\" para ver os comandos.");
        erro = 1;
        break;
    }
...
void info() {
    ...
    puts("Uma calculadora simples que realiza os calculos na ordem que são inseridos. Tem o objetivo de ajudar jogadores de rpg a fazer calculos simples com agilidade.\nPara mais informação de como dar os comandos digite: -help\n");
}
...
void help() {
    ...
    puts("Como calcular:");
    puts("insira o numero, o operador e outro numero, caso queira outra operação a partir do resultado da operação anterior é só digitar outro operador e outro valor. A operação incerra com o operador igual (=). As contas são realizadas na ordem das entradas\n");
    puts("Quais operações são validas:");
    puts("soma +\nsubtração -\nmultiplicação *\ndivisão /\nexponenciação ^\nporcentagem %\n");
}
